...
class MainWindow(Tk.Frame):
    def __init__(self, model, root):
        ...
        sounds_frame.columnconfigure(2, weight=0)

        self.sound_file_path_vars = [Tk.StringVar() for i in range(self.model.get_num_sounds())] 
        self.sound_pin_name_vars = [Tk.StringVar() for i in range(self.model.get_num_sounds())]
        self.sound_pin_option_menus = []

        #group1 = Tk.Frame(self.root, padx=5, pady=5)
        #group1.grid(row=1, column=0, columnspan=3, sticky=Tk.E+Tk.W+Tk.N+Tk.S)
        ...
        # sound number
        sound_label= Tk.Label(sounds_frame, text="Sound " + str(sound_number), justify=Tk.LEFT, anchor="w").grid(sticky=Tk.W, row=row_counter, column=0, columnspan=3)

        # sound file selection
        sound_file_label = Tk.Label(sounds_frame, text="File", justify=Tk.LEFT, anchor="w").grid(sticky=Tk.W, row=1, column=0)

        self.sound_file_path_vars[sound_number-1].set(self.model.get_file_path_for_sound(sound_number))
        sound_file_path_entry = Tk.Entry(sounds_frame, textvariable=self.sound_file_path_vars[sound_number-1], state='readonly').grid(sticky=Tk.W+Tk.E, row=1, column=1)
